...
<template>
  <div
    class="icon-element"
    :class="{ ...elementClasses, selected: isElementSelected && !preview }"
    :style="{
  ...
  >
    ...
    <div 
      v-if="element.locked && isElementSelected && !preview" 
      class="lock-icon"
    ...
    <!-- Show handles only when selected and not locked and not in preview mode -->
    <template v-if="(isElementSelected || isResizing || isRotating) && !element.locked && !preview">
      <!-- Resize Handles -->
      <div
