<template>
  <div
    ...
    :style="{
      ...
      filter: element.hasShadow ? `drop-shadow(${element.shadowOffsetX}px ${element.shadowOffsetY}px ${element.shadowBlur}px ${element.shadowColor})` : 'none',
    }"
    ...
    updateSvgContent(element, newWidth, newHeight) {
      ...
      switch (element.shapeType) {
        ...
        case "starburst": {
          const path = svgElement.querySelector("path");
          if (path) {
            // Scale the original starburst path exactly as it appears in the store
            // Original path: "M50,0L59.7057,13.7778L75,6.6988L76.5165,23.4834L93.3012,25L86.2222,40.2943L100,50L86.2222,59.7057L93.3012,75L76.5165,76.5165L75,93.3012L59.7057,86.2222L50,100L40.2943,86.2222L25,93.3012L23.4834,76.5165L6.6988,75L13.7778,59.7057L0,50L13.7778,40.2943L6.6988,25L23.4834,23.4834L25,6.6988L40.2943,13.7778L50,0Z"
            
            const scaleX = newWidth / 100;
            const scaleY = newHeight / 100;
            
            // Parse the original path and scale each coordinate
            const originalPath = "M50,0L59.7057,13.7778L75,6.6988L76.5165,23.4834L93.3012,25L86.2222,40.2943L100,50L86.2222,59.7057L93.3012,75L76.5165,76.5165L75,93.3012L59.7057,86.2222L50,100L40.2943,86.2222L25,93.3012L23.4834,76.5165L6.6988,75L13.7778,59.7057L0,50L13.7778,40.2943L6.6988,25L23.4834,23.4834L25,6.6988L40.2943,13.7778L50,0Z";
            
            // Scale the path by replacing coordinates
            const scaledPath = originalPath.replace(/(\d+(?:\.\d+)?),(\d+(?:\.\d+)?)/g, (match, x, y) => {
