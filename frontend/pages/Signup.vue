    ...
    <div class="flex items-center justify-center p-12">
      <div class="mx-auto w-[350px] bg-white">
        <div class="mb-8 space-y-1 flex flex-col justify-center align-center">
          <h1 class="text-center text-3xl text-gray-700 font-bold">Register with Ainfographic.com</h1> 
          <h4 class="text-center text-sm text-gray-700">Or <nuxt-link class="text-indigo-500" to="/login">login</nuxt-link>  to your account</h4> 
        </div>
        <form @submit.prevent="handleSignup">
          ...
          <div class="mt-8">
            <button
              class="hover:shadow-form w-full rounded-md bg-indigo-500 py-3 px-8 text-center text-base font-semibold text-white outline-none flex items-center justify-center"
              :disabled="loading"
            >
        ...
        <div class="mb-2">
          <a
            :href="googleAuthLink"
            class="flex items-center justify-center w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-indigo-500 focus:shadow-md"
          >
            <GoogleIcon name="mdi:google" alt="Google" class="mr-2 h-5 w-5 text-gray-500" />
          </a>
        </div>
        </form>
      </div>
...
export default {
  setup() {
     useHead({
      ...
      meta: [
        { name: 'description', content: 'This is the signup page of Ainfographic.' },
        { property: 'og:title', content: 'Sign up | Ainfographic' },
        ...
        { property: 'og:url', content: 'https://ainfographic.com/signup' },
        { name: 'twitter:card', content: 'summary_large_image' },
        { name: 'twitter:title', content: 'Sign up | Ainfographic' },
  ...
  methods: {
    handleSignup(e) {
      ...
      apiClient.post("/account/request-magic-link/", {
        email: this.email,
      })
        .then((response) => {
          if (response.data.message === 'Magic link sent') {
            this.toastStore.addToast({ message: 'Account created! Check your email for a magic link to log in', type: 'success' });
          } else {
        ...
        .catch((error) => {
          if (error.response && error.response.data && error.response.data.error) {
            ...
            this.toastStore.addToast({ message: 'An error occurred during signup', type: 'error' });
          }
