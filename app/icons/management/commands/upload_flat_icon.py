# File: yourapp/management/commands/upload_svgs_to_flaticon.py
...
class Command(BaseCommand):
    help = 'Uploads SVG files to R2 storage and adds them to the FlatIcon model'
    ...
    def handle(self, *args, **options):
        ...
        for i, svg_file in enumerate(svg_files):
            try:
                ...
                unique_filename = f"flat_icons/{str(uuid.uuid4())}.svg"
                ...
                if not dry_run:
                    ...
                    with open(file_path, 'rb') as file_obj:
                        ...
                        if cdn_url:
                            # Create database entry in FlatIcon model
                            flat_icon = FlatIcon.objects.create(
                                title=title,
                                cdn_url=cdn_url,
                                file_format=file_format
                            )
                            self.stdout.write(self.style.SUCCESS(f"  Uploaded and added to FlatIcon with ID: {flat_icon.id}"))
                        else:
