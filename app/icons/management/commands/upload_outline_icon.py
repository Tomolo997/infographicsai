# File: yourapp/management/commands/upload_outline_icon.py
...
from icons.models import OutlineIcon
from io import BytesIO

class Command(BaseCommand):
    help = 'Uploads SVG files to R2 storage and adds them to the OutlineIcon model'
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=str,
            default='svg_icons',
            help='Directory containing SVG files (default: svg_icons)'
        )
        parser.add_argument(
            '--limit',
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        for i, svg_file in enumerate(svg_files):
            try:
                ...
                filename_without_extension = os.path.splitext(svg_file)[0]
                # Remove leading underscore if present
                if filename_without_extension.startswith(' _'):
                    filename_without_extension = filename_without_extension[2:].strip()
                    
                title = filename_without_extension.title()
                
                # Generate a unique filename for R2
                unique_filename = f"outline_icons/{str(uuid.uuid4())}.svg"
                ...
                if not dry_run:
                    ...
                    with open(file_path, 'rb') as file_obj:
                        ...
                        if cdn_url:
                            # Create database entry in OutlineIcon model
                            outline_icon = OutlineIcon.objects.create(
                                title=title,
                                cdn_url=cdn_url,
                                width=width,
                                height=height
                            )
                            self.stdout.write(self.style.SUCCESS(f"  Uploaded and added to OutlineIcon with ID: {outline_icon.id}"))
                        else:
